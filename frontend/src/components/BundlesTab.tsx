...
export function BundlesTab({ bundles, onAddToCart, isLoading = false, error = null }: BundlesTabProps) {
    const categoryColors: Record<string, string> = {
        outfit: 'bg-blue-500/20 text-blue-300 border-blue-500/30',
        layered: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30',
        ...
        'tech-bundle': 'bg-teal-500/20 text-teal-300 border-teal-500/30',
        'skincare-routine': 'bg-rose-500/20 text-rose-300 border-rose-500/30',
    ...
    const formatCategory = (category: string) => {
        return category ? category.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Bundle'
    }
                                        ...
                                        <div className="flex-1 p-4">
                                            <div className="flex items-start justify-between mb-2">
                                                <div className="flex-1">
                                                    <span
                                                        className={`inline-flex items-center gap-1 text-xs mb-2 px-2 py-1 rounded-lg border ${getCategoryColor(bundle.category)}`}
                                                    >
